...
def _render_pr_comment(ctx: Dict) -> str:
    ...
    if bc > 0:
        lines.append("## Delimit: Breaking Changes Detected")
    else:
        lines.append("## Delimit: API Changes Look Good")
    lines.append("")

    lines.append(f"| Metric | Value |")
    lines.append(f"|--------|-------|")
    lines.append(f"| Semver bump | `{bump}` |")
    ...
    if additive:
        lines.append("<details>")
        lines.append("<summary>New additions</summary>")
        lines.append("")
        for c in additive:
            lines.append(f"- `{c['path']}` — {c['message']}")
        lines.append("")
    ...
    lines.append("---")
    lines.append("*Generated by [Delimit](https://github.com/delimit-ai/delimit)*")
    return "\n".join(lines)
